...
with st.sidebar:
    ...
    theme_mode = st.radio("Appearance Mode", ["Light Mode ☀️", "Dark Mode 🌙"], index=0)
...
if is_dark:
    ...
    :root {
      ...
      --forest: #38bdf8; --forest-dim: #0284c7; --gold: #f59e0b; --sage: rgba(56, 189, 248, 0.1);
      --sage-line: rgba(56, 189, 248, 0.2); --clay: #ef4444; --clay-soft: rgba(239, 68, 68, 0.1);
...
<style>
  ...
  /* RESUME PREVIEW CARD STYLING (NO JSON SHOWN) */
  .resume-card {{
...
def generate_base_resume(llm, raw_text, role):
    ...
    except: return {"Name": role, "Summary": t, "Skills": [], "Experience": "", "Education": ""}
...
def render_resume_card(resume_obj):
    """Renders a clean visual resume card instead of showing raw JSON."""
    if not isinstance(resume_obj, dict):
        st.write(resume_obj)
        return
        
    name = resume_obj.get("Name", "Applicant")
    summary = resume_obj.get("Summary", "")
    skills = resume_obj.get("Skills", [])
    exp = resume_obj.get("Experience", "")
    ...
    if isinstance(skills, list):
        ...
        skills_html = f"<span>{skills}</span>"

    st.markdown(f"""
    <div class="resume-card">
        <div class="resume-name">{name}</div>
        <p style="color: var(--ink-soft); font-size: 0.88rem; line-height: 1.5; margin-top: 8px;">{summary}</p>
        
        <div class="resume-section-title">Core Competencies & Skills</div>
        <div style="margin-bottom: 12px;">{skills_html}</div>
        
        <div class="resume-section-title">Experience</div>
        <p style="font-size: 0.85rem; color: var(--ink-soft); font-family: 'Inter', sans-serif;">{exp}</p>
        
        <div class="resume-section-title">Education</div>
        <p style="font-size: 0.85rem; color: var(--ink-soft); font-family: 'Inter', sans-serif;">{edu}</p>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    <div class="page-head">
      <div>
        ...
        <div class="page-desc">Generate a baseline structured profile that serves as the foundation for tailoring.</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with colB:
        if st.session_state.base_resume:
            render_resume_card(st.session_state.base_resume)
        else: 
            st.info("Your polished profile preview will appear here.")
                ...
                if st.button(f"Tailor Resumes & Prepare Submission", key=f"btn_{e_type}"):
                    if not st.session_state.base_resume: st.error("Generate Base Resume first (Step 01)")
                        ...
                        for j in selected_for_apply:
                            tailored = tailor_resume(get_llm(), st.session_state.base_resume, j['desc'])
                            st.session_state.tailored_resumes[j['title']] = tailored
                            st.success(f"Tailored resume saved for: {j['company']}")
